...
import 'package:incite/utils/deep_link.dart';
...
void main()async {
  ...
  OneSignal.shared.setAppId("6ccfdd69-dd80-4e87-8521-ec4fab8e6ed5"); 
          ...
          // print(list.type);
            ...
            if (list.type == 'quote') {
              ...
              SchedulerBinding.instance.addPostFrameCallback((timeStamp) async{
               ...
               arguments: [0,false,PreloadPageController(),"noti${list.id}",
               action == 'Share' ? BlogOptionType.share : null, true]);
              });
            } else {
               if (list.images!.isNotEmpty) {
                  WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
                    await precacheImage(CachedNetworkImageProvider(list.images![0]), context);
                  });
                }
             if (buttons.isNotEmpty && action == 'Bookmark') {
                updateBlogList(list, provider);
              WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
               Navigator.pushNamed(navkey.currentState!.context, '/BlogWrap',arguments: [0,false,PreloadPageController(),"noti${list.id}",BlogOptionType.bookmark,true]);
              });
              ...
              updateBlogList(list, provider);
              WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
               Navigator.pushNamed(navkey.currentState!.context, '/BlogWrap',arguments: [0,false,PreloadPageController(),"noti${list.id}",BlogOptionType.share,true]);
              });
              ...
              // ignore: use_build_context_synchronously
               updateBlogList(list, provider);
               WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
                  Navigator.pushNamed(navkey.currentState!.context, '/BlogWrap',arguments: [0,false,PreloadPageController(),"noti${list.id}",null,true]);
               });
             } 
            }
            }
                    ...
                    return AnnotatedRegion<SystemUiOverlayStyle>(
                            ...
                            return child;
                             ...
                             theme: value.isDarkModeEnabled.value
                              ? getDarkThemeData(hexToRgb(setting.primaryColor),hexToRgb(setting.secondaryColor))
                              : getLightThemeData(hexToRgb(setting.primaryColor),hexToRgb(setting.secondaryColor)),
                            )
                    );
                  }
                );
     ...
     Positioned(
      bottom: 0,
       child: Container(
        ...
        child: Padding(
          padding: const EdgeInsets.only(left: 30,right: 30,top: 32,bottom: 12),
          child: Column(
